...
首先，我們啟動一個程式，它會從命令列讀取你的命令，然後執行。你可以輸入`push <value>`，將一個元素入隊，輸入`pop`，將一個元素彈出。另外，你還可以使用這個程式啟動多個例項，用來模擬分散式的環境：
...
1.  **Barrier：分散式柵欄**。如果持有 Barrier 的節點釋放了它，所有等待這個 Barrier 的節點就不會被阻塞，而是會繼續執行。
...
1.  **Hold 方法**是建立一個 Barrier。如果 Barrier 已經建立好了，有節點呼叫它的 Wait 方法，就會被阻塞。
2.  **Release 方法**是釋放這個 Barrier，也就是開啟柵欄。如果使用了這個方法，所有被阻塞的節點都會被放行，繼續執行。
3.  **Wait 方法**會阻塞當前的呼叫者，直到這個 Barrier 被 release。如果這個柵欄不存在，呼叫者不會被阻塞，而是會繼續執行。
...
好了，我們先來簡單總結一下。我們在第 17 講學習的迴圈柵欄，控制的是同一個程式中的不同 goroutine 的執行，而**分散式柵欄和計數型柵欄控制的是不同節點、不同行程的執行**。當你需要協調一組分散式節點在某個時間點同時執行的時候，可以考慮 etcd 提供的這組併發原語。
...
1.  部署一個 3 節點的 etcd 叢集，測試一下分散式佇列的效能。
2.  etcd 提供的 STM 是分散式事務嗎？
