...
為了實作順序的資料傳遞，我們可以定義一個令牌的變數，誰得到令牌，誰就可以列印一次自己的編號，同時將令牌**傳遞**給下一個 goroutine，我們嘗試使用 chan 來實作，可以看下下面的程式碼。
...
1.  takeN：只取流中的前 n 個資料；
2.  takeFn：篩選流中的資料，只保留滿足條件的資料；
3.  takeWhile：只取前面滿足條件的資料，一旦不滿足條件，就不再取；
4.  skipN：跳過流中前幾個資料；
5.  skipFn：跳過滿足條件的資料；
6.  skipWhile：跳過前面滿足條件的資料，一旦不滿足條件，當前這個元素和以後的元素都會輸出給 Channel 的 receiver。
